...
class ROBERTA(nn.Module):
    ...
    def get_emotion_vec(self, features, split_layer: int = 6):
        """
        提取情感潜在表示，默认使用第6层的输出作为编码器的隐层表示，可以根据需要调整split_layer参数。

        Args:
            features: 包含输入特征的元组，具体包含的信息和之前提取主题表示时一致。
            split_layer: 指定使用哪一层的隐层输出，默认值为6。

        Returns:
            emotion_vecs: 提取的情感潜在表示张量。
        """
        # 获取批次大小和最大序列长度
        batch_size, max_seq_len_ex, max_text_seq_len = features[0].size()
        seqlens = features[2]

        # 初始化列表存储每个样本的uttrs和masks
        lst_uttrs = []
        lst_masks = []
        ...
        # 取出CLS token的表示作为情感表示
        emotion_vecs = output_tokens[:, 0, :]
        emotion_vecs = emotion_vecs.to(self.devicepad_device)

        we_dim = self.get_word_embedding_dimension()

        for ibatch in range(batch_size):
            fullzeropad4insert = torch.zeros(
                max_seq_len_ex - seqlens[ibatch], we_dim).to(self.devicepad_device)
            index4insert = ibatch * max_seq_len_ex + seqlens[ibatch]
            emotion_vecs = torch.cat(
                [emotion_vecs[:index4insert],
                 fullzeropad4insert,
                 emotion_vecs[index4insert:]], dim=0)

        emotion_vecs = emotion_vecs.view(batch_size, max_seq_len_ex, we_dim)
